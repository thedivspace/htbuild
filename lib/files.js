...
function scanAssets(path='.\\source'){
    ...
    fs.readdirSync(path).forEach(file => {
        if(fs.statSync(path+"\\"+file).isDirectory()){
            ...
            var isHtml = (splitArray[splitArray.length-1].toUpperCase()) == 'HTML'||'PHP'||'HTM';
            if(!isHtml)
                assets.push(path+"\\"+file);
        }
    });
...
function isHtml(path) {
    ...
    var extension = splitArray[splitArray.length-1].toUpperCase()
    return (extension == 'HTML' || "HTM" || 'PHP' );
}
